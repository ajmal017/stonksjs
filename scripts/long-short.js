...
class LongShort {
  ...
  async getStockList() {
    try {
      ...
      console.log(`Taking position in ${this.symbols.toString()}`);
  ...
  async rebalance() {
    ...
    try {
      ...
      if (!isNaN(this.qShort) && !isNaN(this.qLong)) {
        ...
        console.log('*****************************************');
        console.warn('Invalid qShort or qLong');
        console.log('*****************************************');
        console.log(this);
        console.log('*****************************************');
      }
  ...
  async submitOrder({ quantity, stock, side }) {
    // console.trace({ quantity, stock, side });
    return new Promise(async (resolve) => {
      const qty = parseFloat(quantity);
      if (qty <= 0) {
        console.log(`Quantity is <=0, order of | ${qty} ${stock} ${side} | not sent.`);
        resolve(true);
        return;
      }
      ...
      try {
        ...
        console.log(`Market order of | ${qty} ${stock} ${side} | completed.`);
        resolve(true);
      } catch (err) {
        console.log(`Order of | ${qty} ${stock} ${side} | did not go through.`);
        resolve(false);
      }
    });
  }
